...
# Tentukan nama image
SHIPPING_IMAGE_NAME=shipping-service

# Perintah untuk build Docker image dari Dockerfile
docker build -t ghcr.io/$REPO_NAME/$SHIPPING_IMAGE_NAME:latest .
...
docker push ghcr.io/$REPO_NAME/$SHIPPING_IMAGE_NAME:latest
